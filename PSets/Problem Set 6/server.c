...
{
    int shift = 0;
    //checks for / on last position
    if (path[strlen(path) -1] == '/')
        shift = 1;
        
    char* newPath = malloc(strlen(path)+11+1);
    strcpy(newPath,path);
    strncpy(newPath+strlen(path)-shift,"/index.php\0",11);
    printf("%s\n",newPath);
    if (fopen(newPath,"r") != NULL)
    {
        return newPath;
    }
    
    strncpy(newPath+strlen(path)-shift,"/index.html\0",12);
    if (fopen(newPath,"r") != NULL)
    {
        return newPath;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    BYTE* contentNow = malloc(sizeof(BYTE)*BYTES);
    int shift = 0;
    int read = 0;
    while (!feof(file))
    {
        read = fread(contentNow+shift,1,BYTES,file);
        shift +=read;
        contentNow = realloc(contentNow, shift+BYTES);
    }
...
{
    char extenstionTemp [strlen(extension)+1];
    strcpy(extenstionTemp,extension);
    
    if (strrchr(extension,'?') != NULL)
    {
        int qMarkPos = strrchr(extension,'?')-extension;
        extenstionTemp[qMarkPos] = 0;
    }    
    
    
    char* extPointer = extenstionTemp;
    if (strrchr(extenstionTemp,'.') !=NULL)
        extPointer = strchr(extension,'.')+1;
    else
        return NULL;
    
    
        
    etT options[] = {   {"css","text/css"},
                        {"html","text/html"},
                        {"gif","image/gif"},
                        {"ico","image/x-icon"},
                        {"jpg","image/jpeg"},
                        ...
                        {"png","image/png"},
                        {"php","text/x-php"} };
    int elements = sizeof(options)/sizeof(options[0]);
                        
    for(int i=0;i<elements;i++)
    {
        etT type = options[i];
        
        if (    strcasecmp(extPointer,type.ext) == 0)
        {
            return type.type;
        }
    }
...
{
    int spaceIndex = (int) (strchr(line,' ')-line);
    if (spaceIndex < 2)
    {
        error(400);
        return false;
    ...
    int spaceIndex2 = (int) (strchr(line+spaceIndex+1,' ')-line);
    if(spaceIndex2 < 0 )
    {
        error(400);
        return false;
    ...
    int crlf = (int) (strchr(line,'\r')-line);
    if(crlf < 0 || line[crlf+1] != '\n' )
    {
        error(400);
        return false;
    }
    ...
    if ( strchr(line+4,'"') != NULL )
    {
        error(400);
        return false;
    }

    if ( strcmp(line+spaceIndex2+1,"HTTP/1.1") == 0 )
    {
         error(505);
        return false;  
    }
    ...
    {
         //printf("%d\n",spaceIndex2-4);
         strncpy(abs_path,line+4,spaceIndex2-4);
         abs_path[spaceIndex2-4] = '\0';
         query[0] = '\0';
    }
    else
    {
        int questionM = strchr(line,'?')-(line);
        strncpy(abs_path,line+4,questionM-4);
        abs_path[questionM-4] = '\0';
        strncpy(query,line+questionM+1,spaceIndex2-questionM-1);
        query[spaceIndex2-questionM-1] = '\0'; 
    }
